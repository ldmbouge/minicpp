...
namespace gfl {
GFL_HOST_DEVICE constexpr i32 popcount(u32 const x) noexcept {
#ifdef __CUDA_ARCH__
...
GFL_HOST_DEVICE constexpr i32 popcount(u64 const x) noexcept {
#ifdef __CUDA_ARCH__
...
// Count leading zeros
GFL_HOST_DEVICE constexpr i32 clz(u32 const x) noexcept {
    assert(x != 0);
#ifdef __CUDA_ARCH__
...
GFL_HOST_DEVICE constexpr i32 clz(u64 const x) noexcept {
    assert(x != 0);
#ifdef __CUDA_ARCH__
...
GFL_HOST_DEVICE constexpr i32 ctz(u32 const x) noexcept {
    assert(x != 0);
#ifdef __CUDA_ARCH__
...
GFL_HOST_DEVICE constexpr i32 ctz(u64 const x) noexcept {
    assert(x != 0);
#ifdef __CUDA_ARCH__
...
// Bit reverse
GFL_HOST_DEVICE constexpr u32 bitreverse(u32 const x) noexcept {
#ifdef __CUDA_ARCH__
...
GFL_HOST_DEVICE constexpr u64 bitreverse(u64 const x) noexcept {
#ifdef __CUDA_ARCH__
...
template<typename T>
GFL_HOST_DEVICE constexpr T mask(i32 const bitIdx) noexcept {
    static_assert(std::is_integral_v<T>);
    static_assert(std::is_unsigned_v<T>);
    assert(bitIdx >= 0);
    assert(bitIdx < gfl::numeric_limits<T>::digits);
    return scast<T>(1) << bitIdx;
}

// Most significant bit index
template<typename T> GFL_HOST_DEVICE constexpr i32 msb(T const x) noexcept {
    static_assert(std::is_integral_v<T>);
    static_assert(std::is_unsigned_v<T>);
    assert(x != 0);
    constexpr i32 bitSize = numeric_limits<T>::digits;
...
template<typename T> GFL_HOST_DEVICE constexpr i32 lsb(T const x) noexcept {
    static_assert(std::is_integral_v<T>);
    static_assert(std::is_unsigned_v<T>);
    assert(x != 0);
    i32 const idx = ctz(x);
    assert(idx >= 0);
    assert(idx < numeric_limits<T>::digits);
    return idx;
}

// Flip most significant bit
template<typename T>
GFL_HOST_DEVICE constexpr T flipMsb(T const x) noexcept {
    static_assert(std::is_integral_v<T>);
    static_assert(std::is_unsigned_v<T>);
    assert(x != 0);
    return x ^ mask<T>(msb(x));
}

// Flip least significant bit
template<typename T> GFL_HOST_DEVICE constexpr T flipLsb(T const x) noexcept {
    static_assert(std::is_integral_v<T>);
    static_assert(std::is_unsigned_v<T>);
    assert(x != 0);
    return x ^ mask<T>(lsb(x));
}

template<typename T>
GFL_HOST_DEVICE constexpr bool test(T const x, i32 const bitIdx) noexcept {
    static_assert(std::is_integral_v<T>);
    static_assert(std::is_unsigned_v<T>);
    assert(bitIdx >= 0);
    assert(bitIdx < numeric_limits<T>::digits);
    return (x & mask<T>(bitIdx)) != 0;
}

// Suffix mask inclusive (bits from bitIdx to LSB)
template<typename T>
GFL_HOST_DEVICE constexpr T suffixMaskInclusive(i32 const bitIdx) noexcept {
    static_assert(std::is_integral_v<T>);
    static_assert(std::is_unsigned_v<T>);
...
template<typename T> GFL_HOST_DEVICE constexpr T prefixMaskInclusive(i32 const bitIdx) noexcept {
    static_assert(std::is_integral_v<T>);
    static_assert(std::is_unsigned_v<T>);
    i32 constexpr bitSize = numeric_limits<T>::digits;
    T constexpr fullMask = numeric_limits<T>::max();
...
template<typename T>
GFL_HOST_DEVICE static void printBits(T const x) noexcept {
    static_assert(std::is_integral_v<T>);
    static_assert(std::is_unsigned_v<T>);
    i32 constexpr bitSize = numeric_limits<T>::digits;
    for (i32 bitIdx = 0; bitIdx < bitSize; bitIdx += 1)
