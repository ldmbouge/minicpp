...
MDD::MDD(CPSolver::Ptr cp, Factory::Veci iv, bool reduced): cp(cp), reduced(reduced), trail(cp->getStateManager()){
...
void MDD::post(){
    ...
    std::vector< std::vector<MDDNode*> > vec(numVariables+1, std::vector<MDDNode*>(0));
    this->layers = vec;
    for(int i = 0; i < numVariables+1; i++){
        ::trail<int> temp(this->trail, 0);
        this->layerSize.push_back(temp);
    }
    std::vector< std::vector<::trail<int>> > sup(numVariables, std::vector<::trail<int>>(0));
    this->supports = sup;
    
    //Create Supports for all values for each variable
    for(int i = 0; i < numVariables; i++){
        //std::vector<::trail<int>> s;
        for(int v = x[i]->min(); v <= x[i]->max(); v++){
            ::trail<int> temp(this->trail, 0);
            supports[i].push_back(temp);
        }
                ...
                if(state != nullptr){
                    ...
                    if(i < numVariables - 1){
                        for(int j = 0; j < layers[i+1].size(); j++){
                            if(*state == *(layers[i+1][j]->getState())){
                                child = layers[i+1][j];
                                break;
                            }
                        }
...
/*
 MDD::removeSupport(int layer, int value) decrements support for value at specific layer. If support for a value reaches 0, then value is removed from the domain.
 */
...
/*
 MDD::saveGraph() prints the current state of the MDD to stdout in dot format. Use a graphviz dot graph visualizer to create a graphical view of the diagram.
 */
void MDD::saveGraph(){
    ...
    for(int l = 0; l < numVariables; l++){
        for(int i = 0; i < layers[l].size(); i++){
            ...
            for(int j = 0; j < nc; j++){
                int count = ch[j]->getChild()->getID();

                std::cout << "Layer_" << l << "_" << i << " ->" << "Layer_" << l+1 << "_" << count << " [ label=\"" << ch[j]->getValue() << "\" ];" << std::endl;
